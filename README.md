...
### (1) ``` clean_data(df, index_name = NONE)```
...
### (6) ```pretty_multi_graph(dfs, labels, title)```
...
### (7) ```mcomp_graph_stacked(df1, lab1, df2, lab2)```
...
### (8) ```comp_graph_stacked(df1, lab1, df2, lab2)```
...
### (9) ```comp_graph_dual(df1, lab1, df2, lab2, show_change=False)```
...
### (10) ```mcomp_graph_dual(df1, lab1, df2, lab2, show_change=False)```  (MAINLY WHAT I USED FOR MY POWERPOINT)
...
### USEAGE 
 ...
 ### (11) ```save_mcomp_graph_dual(df1, lab1, df2, lab2, show_change=False)```  (FOR SAVING ONLY)
